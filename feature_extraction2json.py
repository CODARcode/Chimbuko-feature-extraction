...
def parse_functions(node, context, thread, infile, data, num_functions, function_map):
    ...
    if num_functions > 0:
        ...
        for i in range(0,num_functions):
            ...
            # calls
            timer.append(int(tokens[0]))
            # Subroutines
            timer.append(int(tokens[1]))
            # Exclusive
            timer.append(int(tokens[2]))
            # Inclusive
            timer.append(int(tokens[3]))
            # Profile Calls
            #timer.append(int(tokens[4]))
...
def main(arguments):
    ...
    print (" The parse arguments are ", args)
